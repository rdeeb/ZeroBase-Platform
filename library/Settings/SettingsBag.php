...
class SettingsBag extends Iterator
{
    ...
    public function getPages( $form_name )
    {
        ...
        foreach ( $this->data as $widget_name => $options )
        {
            ...
            {
                $pages[ $options[ 'section' ] ] = FormFactory::createForm( $form_name . '-' . $options[ 'section' ],
                    'options', 'option' );
            }
            $pages[ $options[ 'section' ] ]->addWidget( $widget_name, $options[ 'widget' ],
                $options[ 'options' ][ 'widget_options' ], get_option( $widget_name,
                    isset( $options[ 'options' ][ 'default' ] ) ? $options[ 'options' ][ 'default' ] : NULL ) );
        }
